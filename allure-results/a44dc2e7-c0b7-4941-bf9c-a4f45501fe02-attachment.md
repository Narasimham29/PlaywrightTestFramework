...
```
Error: Searched item "Macbook" not found in results
```
...
> 36 |         throw new Error(`Searched item "${searchItem}" not found in results`);
  ...
  40 |         await this.searchresults.first().scrollIntoViewIfNeeded();
  41 |         let searchItems = await this.searchresults.all();
  42 |         for(let item of searchItems){
  43 |             const text = (await item.textContent())?.trim();
  44 |         if (text?.toLocaleLowerCase()?.includes(searchItem.toLocaleLowerCase())) {
  45 |                 return true;
  46 |             }
  47 |         }
  48 |         return false;
