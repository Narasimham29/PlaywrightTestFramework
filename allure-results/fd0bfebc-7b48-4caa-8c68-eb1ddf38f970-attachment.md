...
```
Error: locator.scrollIntoViewIfNeeded: Error: strict mode violation: locator('div.caption a') resolved to 3 elements:
    1) <a href="https://naveenautomationlabs.com/opencart/index.php?route=product/product&product_id=43&search=Macbook">MacBook</a> aka getByText('MacBook', { exact: true })
    2) <a href="https://naveenautomationlabs.com/opencart/index.php?route=product/product&product_id=44&search=Macbook">MacBook Air</a> aka getByText('MacBook Air', { exact: true })
    3) <a href="https://naveenautomationlabs.com/opencart/index.php?route=product/product&product_id=45&search=Macbook">MacBook Pro</a> aka getByText('MacBook Pro')

Call log:
  - waiting for locator('div.caption a')
...
```ts
  ...
  3  | 
  4  | export class SearchResults{
  5  |     readonly page : Page;
  6  |     
  7  |     readonly searchheader : Locator;
  8  |     readonly searchresults : Locator;
  9  |     
  10 | 
  11 | 
  12 |     constructor(page: Page){
  13 |         this.page = page;
  14 |         this.searchheader = page.locator("div#content h1");
  15 |         
  16 |         this.searchresults = page.locator("div.caption a")
  17 |         
  18 |     }
  19 |     async isSearchPageExists(){
  20 |         let text = await this.searchheader.textContent()
  21 |         if(text?.includes("Search -")){
  22 |             return true
  23 |         }else{
  24 |             return false
  25 |         }
  26 |     }
  27 |     
  28 |     async selectSearchItem(searchItem:string) : Promise<ProductPage>{
  29 |         let searchItems = await this.searchresults.all()
  30 |         for(let item of searchItems){
  31 |             if(await item.textContent() == searchItem){
  32 |                 await item.click();
  33 |                 return new ProductPage(this.page)
  34 |             }
  35 |         }
  36 |         throw new Error(`Searched item "${searchItem}" not found in results`);
  37 |     }
  38 | 
  39 |     async isSearchItemExist(searchItem:string){
> 40 |         await this.searchresults.scrollIntoViewIfNeeded();
     |                                  ^ Error: locator.scrollIntoViewIfNeeded: Error: strict mode violation: locator('div.caption a') resolved to 3 elements:
  41 |         let searchItems = await this.searchresults.all();
  42 |         for(let item of searchItems){
  43 |             if(await item.textContent() == searchItem){
  44 |                 return true;
  45 |             }
  46 |         }
  47 |         return false;
